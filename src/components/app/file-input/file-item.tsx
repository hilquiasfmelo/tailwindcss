...
const fileItem = tv({
  // Define slots quando a estilização será para mais e um componente.
  slots: {
    container:
  ...
  variants: {
    state: {
      progress: {
        // Se não houver estilização a mais, será adquirido a padrão container.
        container: '',
      },
      complete: {
    ...
    <div className={container()}>
      ...
      {state === 'error' ? (
        <div className="flex flex-1 flex-col items-start gap-1">
          ...
          <button
            type="button"
            className="text-error-700 hover:text-error-900 font-semibold"
          >
            Tentar novamente
          </button>
        </div>
